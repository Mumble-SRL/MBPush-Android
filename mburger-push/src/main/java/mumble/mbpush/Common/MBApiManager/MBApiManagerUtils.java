...
import mumble.mbpush.BuildConfig;
import mumble.mbpush.Common.MBConstants.MBConstants;

/**
 * Utilities with the {@link MBAPIManager3 MBAPIManager3 class}
 *
 * @author Enrico Ori
 * @version {@value MBConstants#version}
 */
public class MBApiManagerUtils {
    ...
    public static boolean hasMapOkResults(Map<String, Object> map, boolean payload) {
        if (map != null) {
            if (map.containsKey(MBApiManagerConfig.AM_RESULT)) {
                if ((int) map.get(MBApiManagerConfig.AM_RESULT) == MBApiManagerConfig.RESULT_OK) {
                    if (!map.containsKey(MBApiManagerConfig.AM_ERROR)) {
                        if (payload) {
                            if (map.containsKey(MBApiManagerConfig.AM_PAYLOAD)) {
                                return true;
                            }
                        } else {
                            return true;
